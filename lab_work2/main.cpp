...
int main() {
    ...
    BFS_DFS_graph.addEdge(7, 6);
    BFS_DFS_graph.addEdge(7, 8);
    BFS_DFS_graph.addEdge(4, 9);
    BFS_DFS_graph.visualizeWithGraphviz("BFS_DFS_graph.png");
...
//    kruskal_prime_graph.addEdge(2, 3, 4);
    ...
    std::vector<std::tuple<int, int, int>> mst_kruskal_edges = kruskal_prime_graph.kruskal();
    auto end = std::chrono::high_resolution_clock::now();
    WeightedGraph mst_kruscal(9);
    for (auto edge: mst_kruskal_edges) {
        mst_kruscal.addEdge(std::get<0>(edge), std::get<1>(edge), std::get<2>(edge));
    }
    mst_kruscal.visualizeWithGraphviz("mst_kruscal.png");
    for (const auto &edge: mst_kruskal_edges) {
        std::cout << "(" << std::get<0>(edge) << ", " << std::get<1>(edge) << ") with weight "
    ...
    int sum_kruskal = std::accumulate(mst_kruskal_edges.begin(), mst_kruskal_edges.end(), 0,
                                      [](int sum, const std::tuple<int, int, int> &edge) {
                                          return sum + std::get<2>(edge);
                                      });
    int sum_prim = std::accumulate(mst_kruskal_edges.begin(), mst_kruskal_edges.end(), 0,
                                   [](int sum, const std::tuple<int, int, int> &edge) {
                                       return sum + std::get<2>(edge);
                                   });
    std::cout << "(Kruskal) Sum of weights in the spanning tree: " << sum_kruskal << "\nTime: "
              << kruskal_time << std::endl;
